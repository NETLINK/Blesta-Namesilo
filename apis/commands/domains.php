...
class NamesiloDomains {
	...
	public function getContacts( array $vars ) {
		...
		$contact_ids = self::getDomainInfo( $vars )->response()->contact_ids;
		//$contacts = array_unique( $contacts );
		$contacts = $temp = array();
		foreach ( $contact_ids as $type => $id ) {
			if ( !isset( $temp[$id] ) ) {
				$temp[$id] = $this->api->submit( "contactList", array( "contact_id" => $id ) )->response()->contact;
		...
		//trigger_error( var_export( $contact, true ) );
		return $contacts;
		*/
	}
	...
	public function setContacts(array $vars) {
		return $this->api->submit("namesilo.domains.setContacts", $vars);	
	}
